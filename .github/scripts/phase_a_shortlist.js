...
(() => TIMEOUT)(); // keep TIMEOUT referenced (some linters)
...
(async () => {
  ...
  for (const seed of seeds) {
    ...
    try {
      ...
      if (Array.isArray(arr)) allRowsRaw.push(...arr);
    } catch {}
  }

  // 🔒 Filter out your real domains (apex or any subdomain)
  const allRows = allRowsRaw.filter(r => r?.domain && !isSeedOrSub(r.domain));

  // --- shortlist logic (do NOT require A/MX here) ---
  const seenCachePath = path.join(".dnstwist-cache","seen.json");
  let seen = [];
  if (fs.existsSync(seenCachePath)) {
    try { seen = JSON.parse(await fsp.readFile(seenCachePath,"utf8")); } catch {}
  }
  ...
  const candidates = rows
    .filter(r => (r._nameDist <= NAME_EDIT_MAX) || (r.http_similarity >= lshThr) || isRiskyName(r.domain) || r._hasMX)
